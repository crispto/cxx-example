...
void Monitor::report(bool time, bool resource)
{
    ...
    if (time) {
        ...
        for (auto &item : func_proc_map_) {
            ...
            for (auto &func : item.second) {
                std::cout << "flag: " << func.flag << " start: " << func.start_time << " end: " << func.end_time << " elapse "
    ...
    if (resource) {
        ...
        for (auto &item : resouce_) {
            std::cout << "ts: " << item.ts << " cpu: " << item.cpu << " gpu: " << item.gpu << " mem: " << item.mem << std::endl;
        }
    }
}
static int64_t get_now_milli()
{
    ...
    int64_t epoch = std::chrono::duration_cast<std::chrono::milliseconds>(now.time_since_epoch()).count();
    return epoch;
}
