...
template <typename T> void step1(std::vector<std::vector<T>> &matrix, int &step)
{
    // process rows
    for (auto &row : matrix) {
...
template <typename T>
void step2(const std::vector<std::vector<T>> &matrix, std::vector<std::vector<int>> &M,
...
template <typename T>
void step4(const std::vector<std::vector<T>> &matrix, std::vector<std::vector<int>> &M,
           std::vector<int> &RowCover, std::vector<int> &ColCover, int &path_row_0,
           ...
           std::vector<int> &ColCover, int &step)
{
    ...
    while (!done) {
        ...
        if (!done) {
            ...
            path_count += 1;
            if (path_count - 1 > static_cast<int>(path.size()) - 1) {
                throw std::runtime_error("访问溢出");
            }
